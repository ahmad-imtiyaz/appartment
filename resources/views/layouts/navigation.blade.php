        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    @php
                        $homeRoute = match(auth()->user()->role ?? 'guest') {
                            'admin' => 'admin.dashboard',
                            'pekerja' => 'worker.tasks.index',
                            default => 'guest.service-requests.index',
                        };
                    @endphp
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    @php
                        $navItems = match(auth()->user()->role ?? 'guest') {
                            'admin' => [
                                ...
                                ['route' => 'admin.product-listings.index', 'label' => 'Jual-Beli'],
                            ],
                            'pekerja' => [
                                ['route' => 'worker.tasks.index', 'label' => 'Tugas Saya'],
                            ],
                            'guest' => [
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                ...
                <div class="mt-1 px-2 py-0.5 text-xs font-medium rounded-full inline-block
                    @if(Auth::user()->role === 'admin') bg-purple-100 text-purple-800
                    @elseif(Auth::user()->role === 'pekerja') bg-blue-100 text-blue-800
                    @else bg-green-100 text-green-800 @endif">
                    {{ ucfirst(Auth::user()->role) }}
