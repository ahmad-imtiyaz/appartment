<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tambah Tier Reward Koin') }}
        </h2>
    </x-slot>

    <div class="py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <form action="{{ route('admin.coin-settings.store') }}" method="POST" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <input type="number" name="coin_reward" value="{{ old('coin_reward') }}" min="1"
                           class="w-full rounded-lg border-gray-300 shadow-sm" required>
                    @error('coin_reward')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
